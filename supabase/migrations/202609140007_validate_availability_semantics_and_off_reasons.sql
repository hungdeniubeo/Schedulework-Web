...
begin
  ...
  for day_key in select generate_series(1, 7)::text loop
    ...
    if off_reason_value is not null
      and jsonb_typeof(off_reason_value) not in ('null', 'string')
    then
      return false;
    end if;
    if jsonb_typeof(off_reason_value) = 'string'
...
comment on function private.is_valid_availability(jsonb) is
  'Validates legacy v1 plus semantic whole-hour v2 availability with optional per-day off reasons.';
